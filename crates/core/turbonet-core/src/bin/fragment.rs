...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    dotenvy::dotenv().ok();
    println!("🔥 TURBONET: DEEP-SEA ASYMMETRIC FRAGMENTER v3.0 ENGAGED...");

    let target_ip = std::env::var("TURBONET_TARGET_IP").expect("TURBONET_TARGET_IP not set");
    let p1_port: u16 = std::env::var("LANE1_PORT")
        .unwrap_or_else(|_| "8001".to_string())
        .parse()
        .unwrap();
    let _p2_port: u16 = std::env::var("LANE2_PORT")
        .unwrap_or_else(|_| "8002".to_string())
        .parse()
        .unwrap();
    let _p3_port: u16 = std::env::var("LANE3_PORT")
        .unwrap_or_else(|_| "8003".to_string())
        .parse()
        .unwrap();

    let w0_env: i32 = std::env::var("SHRED_W0")
        .unwrap_or_else(|_| "10".to_string())
        .parse()
        .unwrap();
    let w1_env: i32 = std::env::var("SHRED_W1")
        .unwrap_or_else(|_| "45".to_string())
        .parse()
        .unwrap();
    let w2_env: i32 = std::env::var("SHRED_W2")
        .unwrap_or_else(|_| "45".to_string())
        .parse()
        .unwrap();
    let block_size: usize = std::env::var("BLOCK_SIZE")
        .unwrap_or_else(|_| "5242880".to_string())
        .parse()
        .unwrap();

    let _manifest_dir = env!("CARGO_MANIFEST_DIR");
    let ptx_src = std::fs::read_to_string("fragment.cu")?;
    let ptx = cudarc::nvrtc::compile_ptx(ptx_src)?;
    let dev = CudaDevice::new(0)?;
    dev.load_ptx(ptx, "fragment", &["fragment_kernel"])?;
    ...
    let multilane_mode =
        std::env::var("TURBONET_MULTILANE").unwrap_or_else(|_| "false".to_string()) == "true";

    let file_path = std::env::var("TURBONET_FILE").unwrap_or_else(|_| "payload.bin".to_string());
    let file_bytes = fs::read(&file_path).unwrap_or_else(|_| {
        panic!(
            "Failed to read '{}'. Set TURBONET_FILE=yourfile.ext",
            file_path
        )
    });
    ...
    println!(
        "⚛️ LATTICE: Requesting Public Key from Lane Receiver at {}:{}...",
        target_ip, p1_port
    );
    socket
        .send_to(b"PK_REQ", format!("{}:{}", target_ip, p1_port))
        .await?;
    let mut pk_buf = [0u8; KYBER_PUBLICKEYBYTES];
    let (n, _) = tokio::time::timeout(
        std::time::Duration::from_secs(5),
        socket.recv_from(&mut pk_buf),
    )
    .await??;
    if n != KYBER_PUBLICKEYBYTES {
        return Err("Invalid PK size".into());
    }

    println!("🤝 HANDSHAKE: Public Key received. Encapsulating secret...");
    let mut rng = rand::thread_rng();
    let (_ct, shared_secret) =
        encapsulate(&pk_buf, &mut rng).map_err(|_| "Encapsulation failed")?;
    let quantum_salt = u64::from_be_bytes(shared_secret[0..8].try_into().unwrap());

    // Level 11 METADATA: Robust Handshake
    println!("📦 LATTICE: Synchronizing Metadata with Lane Receiver...");
    ...
    while !meta_confirmed {
        socket
            .send_to(&meta, format!("{}:{}", target_ip, p1_port))
            .await?;
    ...
    println!("✅ SUCCESS: Metadata synchronized. Lane is ready.");

    // LEVEL 7/8: AUTO-PILOT & NEURAL STRATEGIST
    let dynamic_mode =
        std::env::var("TURBONET_DYNAMIC").unwrap_or_else(|_| "false".to_string()) == "true";
    let mut rtts = [0f64; 3];
    ...
    let (w0, w1, w2) = if dynamic_mode {
        ...
        socket
            .send_to(&probe_buf, format!("{}:{}", target_ip, p1_port))
            .await?;
        let mut echo_buf = [0u8; 16];
        match tokio::time::timeout(
            std::time::Duration::from_millis(1000),
            socket.recv_from(&mut echo_buf),
        )
        .await
        {
            Ok(_) => {
        ...
        let use_deepseek_api = std::env::var("TURBONET_DEEPSEEK_API")
        ...
        if use_deepseek_api {
            ...
            let model =
                std::env::var("OLLAMA_MODEL").unwrap_or_else(|_| "deepseek-r1:8b".to_string());
            ...
            if let Some(ai_weights) = get_ai_strategy(rtts).await {
                ...
                (
                    ai_weights.w0 as i32,
                    ai_weights.w1 as i32,
                    ai_weights.w2 as i32,
                )
            } else {
            ...
            println!(
                "⚡ LOCAL PREDICTOR: {}/{}/{} (latency: {:?})",
                weights.w0, weights.w1, weights.w2, latency
            );
            (weights.w0 as i32, weights.w1 as i32, weights.w2 as i32)
        }
    } else {
        println!(
            "⚡ FRAGMENTER CORE: Using static weights {}/{}/{}",
    ...
    for block_idx in 0..total_blocks {
        ...
        let turbo_mode =
            std::env::var("TURBONET_TURBO").unwrap_or_else(|_| "false".to_string()) == "true";
        let adaptive_mode =
            std::env::var("TURBONET_ADAPTIVE").unwrap_or_else(|_| "true".to_string()) == "true";
        ...
        for chunk in block_data.chunks(chunk_size) {
            ...
            sockets[lane_idx]
                .send_to(chunk, &lane_addrs[lane_idx])
                .await?;
            total_packets_sent += 1;
    ...
    if !end_confirmed {
        println!(
            "⚠️ WARNING: No confirmation from receiver (timeout). Data may still be in transit."
        );
    }
