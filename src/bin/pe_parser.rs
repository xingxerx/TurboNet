...
fn analyze_pe(path: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("║  PE Offset:       0x{:08X}                                   ║", pe_offset);
    ...
    let timestamp = u32::from_le_bytes([coff_header[4], coff_header[5], coff_header[6], coff_header[7]]);
    ...
    println!("║  COFF HEADER                                                  ║");
    println!("╠═══════════════════════════════════════════════════════════════╣");
    println!("║  Machine:         0x{:04X} ({})                         ║", machine, arch);
    ...
    if optional_header_size > 0 {
        ...
        let entry_point = u32::from_le_bytes([opt_header[16], opt_header[17], opt_header[18], opt_header[19]]);
        let image_base: u64 = if is_pe32_plus {
            ...
            u32::from_le_bytes([opt_header[28], opt_header[29], opt_header[30], opt_header[31]]) as u64
        };
        
        let subsystem_offset = if is_pe32_plus { 68 } else { 64 };
        let subsystem = u16::from_le_bytes([opt_header[subsystem_offset], opt_header[subsystem_offset + 1]]);
        ...
        println!("║  Magic:           0x{:04X} ({})                          ║", 
                 magic, if is_pe32_plus { "PE32+" } else { "PE32" });
        println!("║  Entry Point:     0x{:08X}                                   ║", entry_point);
        println!("║  Image Base:      0x{:016X}                       ║", image_base);
        println!("║  Subsystem:       {} ({})                              ║", subsystem, subsystem_name);
    }
    ...
    for _ in 0..num_sections {
        ...
        println!("║  {:8} 0x{:08X} 0x{:08X} 0x{:08X} {:5.2}{} {:6} ║",
...
fn print_characteristics(chars: u16) {
    if chars & 0x0002 != 0 { println!("║    - EXECUTABLE_IMAGE                                         ║"); }
    if chars & 0x0020 != 0 { println!("║    - LARGE_ADDRESS_AWARE                                      ║"); }
    if chars & 0x0100 != 0 { println!("║    - 32BIT_MACHINE                                            ║"); }
    if chars & 0x2000 != 0 { println!("║    - DLL                                                      ║"); }
}

fn format_section_flags(chars: u32) -> String {
    let mut flags = String::new();
    if chars & 0x20000000 != 0 { flags.push('X'); } else { flags.push('-'); }
    if chars & 0x40000000 != 0 { flags.push('R'); } else { flags.push('-'); }
    if chars & 0x80000000 != 0 { flags.push('W'); } else { flags.push('-'); }
    flags
}
